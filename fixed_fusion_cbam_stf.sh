...
--model efficientdetv2_dt --batch-size=8 --branch fusion --att_type cbam | tee fixed_fusion_cbam_stf.txt

echo "" | tee -a fixed_fusion_cbam_stf.txt

python validate_all_att.py /groups/ARCL/rgb_gated_aligned --dataset stf_clear_day \
--checkpoint /home/sdeevi/Research/deep-sensor-fusion-main/output/train_stf/EXP_STF_FULL_CBAM/model_best.pth.tar \
--split test --num-classes 4 \
--model efficientdetv2_dt --batch-size=8 --branch fusion --att_type cbam | tee -a fixed_fusion_cbam_stf.txt
