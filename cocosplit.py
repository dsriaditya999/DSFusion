...
def main(args):
    ...
    with open(args.annotations, 'rt', encoding='UTF-8') as annotations:
        ...
        if args.multi_class:
            ...
            save_coco(args.train, type_, filter_images(images, X_train.reshape(-1)), X_train.reshape(-1).tolist(), categories)
            save_coco(args.test, type_,   filter_images(images, X_test.reshape(-1)), X_test.reshape(-1).tolist(), categories)
            ...
            save_coco(args.train, type_, X_train, anns_train, categories)
            save_coco(args.test, type_, X_test, anns_test, categories)
